...
export const postPasswordReset = async (
    _: ResetPasswordStep2Response,
    formData: FormData,
): Promise<ResetPasswordStep2Response> => {
    ...
    try {
        const response = await fetch(
            `${process.env.CC_API_URL}/accounts/reset-password/${parsed.data.token}`,
            {
                method: "POST",
                headers: {
        ...
        if (!response.ok) {
            ...
            if (isFieldError(errorResponse) && errorResponse.error.token) {
                return {
                    ...
                    error: {
                        status: 400,
                        error: "Your link has expired. Please request a new one.",
                    },
                };
            }

            if (isFieldError(errorResponse) || isGeneralError(errorResponse)) {
                return { success: false, error: errorResponse };
            }

            return {
                success: false,
                error: { status: 400, error: "An unexpected error occurred" },
            };
        }
