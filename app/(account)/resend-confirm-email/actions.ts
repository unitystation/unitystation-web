...
export const postResendConfirmationMail = async (
    _: ResendConfirmationMailRequest,
    formData: FormData,
): Promise<ResendConfirmationMailRequest> => {
    ...
    try {
        const response = await fetch(
            `${process.env.CC_API_URL}/accounts/resend-account-confirmation`,
            {
                method: "POST",
                headers: {
        ...
        if (!response.ok) {
            ...
            return {
                success: false,
                error: { error: "An unexpected error happened", status: 500 },
            };
        }
    } catch {
