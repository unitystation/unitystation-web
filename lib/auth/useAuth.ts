...
import {
    serverRequestLoginWithCredentials,
    serverTrySessionLogin,
    serverLogout,
} from "./authorization";
import { z } from "zod";
...
export const useAuth = (): AuthContext => {
    ...
    const clientRequestLoginWithCredentials = async (email: string, password: string) => {
        ...
        if (!parsed.success) {
            const fieldErrors = parsed.error.errors.reduce(
            ...
            setState({
                ...
                state: {
                    isLoggedIn: false,
                    authContext: undefined,
                    error: { error: fieldErrors, status: 400 },
                },
            });
            return;
        }

        const response = await serverRequestLoginWithCredentials(
            parsed.data.email,
        ...
        if (isLoginResponse(response)) {
            setState({
                ...state,
                state: { isLoggedIn: true, authContext: response, error: undefined },
            });
            return;
        } else {
            if (isGeneralError(response)) {
                setState({
                    ...state,
                    state: { isLoggedIn: false, authContext: undefined, error: response },
                });
                return;
            }
        ...
        setState({
            ...
            state: {
                isLoggedIn: false,
                authContext: undefined,
                error: { error: "Unexpected error happened", status: 500 },
            },
        });
    };
    const clientRequestLogout = async () => {
        setState({
            ...state,
            state: { isLoggedIn: false, authContext: undefined, error: undefined },
        });
        await serverLogout();
    };
    ...
    useEffect(() => {
        ...
        // Mount-only session check; state is intentionally excluded to run this exactly once.
